...
Apply it when editing `index.html`, `script.js`, `sandbox.html`, and related mock flow.
...
- Main workflow:
  ...
  3. transfer final `index.html` and `script.js` into HubCloud.
- Local test URL:
...
- Use and extend existing CSS variables in `:root`:
  - `--hc-blue`
  - `--hc-blue-dark`
  - `--hc-blue-soft`
  - `--hc-border`
  ...
  - `--hc-muted`
  - `--hc-bg`
- New colors should be added as variables near existing tokens.
...
- The "rows" KPI card is interactive:
  - hover support,
  - click support,
  - keyboard support (`Enter`/`Space`).
- Ingredient hover modal requirements:
  - opens near KPI card,
  - closes predictably (mouseleave delay, close button, `Esc`),
  - repositions safely on resize/scroll,
  - contains printable ingredient summary.
- Printing behavior:
  - use print-preview overlay,
  - avoid abrupt direct print dialogs.
...
5. Final files remain transferable: `index.html` and `script.js`.
